...
for (var i = 0; i < femaleCharacters.length; i++) {
  var indexToRemove = characters.indexOf(femaleCharacters[i]);
  if (indexToRemove !== -1) {
    characters.splice(indexToRemove, 1);
  }
}

console.log("Length di characters prima dell'operazione:", characters.length);

console.log("Length di characters dopo l'operazione:", characters.length);
/* --EXTRA-- ESERCIZIO 10
...
function printRandomCharacterInfo() {
  ...
  console.log("Ecco alcune informazioni su un personaggio casuale di Star Wars:");
  for (var key in randomCharacter) {
    console.log(key + ":", randomCharacter[key]);
  }
}
